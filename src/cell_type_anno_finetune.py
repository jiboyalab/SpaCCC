...
if __name__ == "__main__":
    ...
    print('>>> input/output representation <<< ', time.ctime())
    # input/output representation
    input_style = "binned"  # "normed_raw", "log1p", or "binned"
    output_style = "binned"  # "normed_raw", "log1p", or "binned"
    print('>>> settings for training <<< ', time.ctime())
    ...
    print('>>> Load and pre-process data <<< ', time.ctime())
    # Step 2: Load and pre-process data
    if dataset_name == "BRCA_Visium_10x_tmp":

        filename = filename #3798 22240
        adata = sc.read(filename) #3798 22240
        adata_ori=adata
        #因为下面涉及到绘图，所以我们先计算umap
        # 运行 PCA
        # sc.pp.pca(adata)
        # # 计算邻居关系
        ...
        # # 计算 UMAP
        # sc.tl.umap(adata)
        # 将adata数据按照4：1的比例划分为refernece数据集和query数据集，用于验证微调模型的效果
        # 设置随机种子，以确保结果可复现
        np.random.seed(43)#重复5次实验，seed从42,43,44,45,46
        # 获取 AnnData 中的观测数目
        n_obs = adata.n_obs
        # 计算训练集和测试集的观测数目
        train_size = int(0.8 * n_obs)
        test_size = n_obs - train_size
        # 随机抽取索引，并将数据集划分为训练集和测试集
        random_indices = np.random.permutation(n_obs)
        train_indices = random_indices[:train_size]
        test_indices = random_indices[train_size:]
        # 根据索引划分 AnnData 数据集
        adata_test = adata[test_indices] #760 22240
        adata = adata[train_indices] #3038 22240

        adata.obs["celltype"] = adata.obs["cell_type"].astype("category")
        adata_test.obs["celltype"] = adata_test.obs["cell_type"].astype("category")
        adata.obs["batch_id"]  = adata.obs["str_batch"] = "0"
        adata_test.obs["batch_id"]  = adata_test.obs["str_batch"] = "1"
        # adata.var.set_index(adata.var["gene_name"], inplace=True) #我们的index已经是gene name了 不需要重新set
    ...
    print('>>> set up the preprocessor, use the args to config the workflow <<< ', time.ctime())
    ...
    print('>>> Load the pre-trained scGPT model <<< ', time.ctime())
    ...
    print('>>> Finetune scGPT with task-specific objectives <<< ', time.ctime())
    ...
    print('>>> Inference with fine-tuned scGPT model <<< ', time.ctime())
    ...
    # The model performance is evaluated on standard classificaton metrics. Here we visualize the predicted labels over the scGPT cell embeddings, and present the confusion matrix for detailed classification performance on the cell-group level.
    ...
    filename="test_cell_type_results.pdf"
    sc.pl.spatial(
            adata_test_raw,
            color=["celltype"],
            show=False, size=1.4, palette=palette_,
            save=filename
        )
    filename="prediction_cell_type_results.pdf"
    sc.pl.spatial(
            adata_test_raw,
            color=["predictions"],
            show=False, size=1.4, palette=palette_,
    ...
    filename="cell_type_results.pdf"
    sc.pl.spatial(
            adata_ori,
            color=["cell_type"],
            show=False, size=1.4, palette=palette_,
    ...
    cm.to_csv(save_dir / "confusion_matrix.csv")

    # plt.figure(figsize=(12, 12))
    # sns.heatmap(cm, annot=True, fmt=".1f", cmap="Blues")
    # plt.savefig(save_dir / "confusion_matrix.pdf", dpi=300)

    # results["test/confusion_matrix"] = wandb.Image(
    #     str(save_dir / "confusion_matrix.pdf"),
    #     caption=f"confusion matrix",
    # )
    print('>>> Save the model into the save_dir <<< ', time.ctime())
